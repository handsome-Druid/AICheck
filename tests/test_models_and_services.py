...
class TestCheckVllmModels(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_check_vllm_models_handles_timeout(self) -> None:
        ...
        zero_timeout_client = AsyncMock()
        zero_timeout_client.timeout.connect = 0.0
        zero_timeout_client.get.side_effect = httpx.TimeoutException("Connect Timeout")

        with patch("src.services.test_vllm.time.time", return_value=100.0):
            zero_timeout_result = await service_module.check_vllm_models(
                client=zero_timeout_client,
                url="https://example.com/chat/completions",
                port=8000,
                container_name="test_c",
                expected_models=["m1"],
        ...
        self.assertEqual(zero_timeout_result.status, "timeout")
        self.assertEqual(zero_timeout_result.response_time, 0.0)
        self.assertIn("请求超时（超过0.0秒）", zero_timeout_result.message)
